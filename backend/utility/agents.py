                ...
                [
                    (
                        ...
                        - **Purchasing Power**: Calculate financial capacity using the formula:
                            ...
                            The filter for price must be ```price <= 1/(Purchasing Power)```
        ...
        if chat_language.lower()=="english":
            prompt = ChatPromptTemplate.from_messages(
                [
                    (
                        "system",
                        """You are an expert real estate sales assistant at NHC Housing Company with ENGLISH native language who talks with the user as a real person based on the user query, conversation history and current information given below.
                        As an expert real estate agent, avoid using short forms like sqm, SAR, etc. Instead use square meters, Saudi Riyals, etc.
                        Always round off the price for eg. 1209856 to 1.2 million.
                        ...
                        2. If user's name is provided in the conversation history, address the user by name.
                        ...
                        4. ONLY USE `current information` to provide information about the properties. NEVER USE INFORMATION NOT PRESENT IN `current information`.
                        ...
                        7. Always provide detailed information about at least 3 records PRESENT ONLY IN THE 'Current Information' and make sure to include the variety on projects.
                        ...
                        9. If `Current Information` is empty, Always ask for more information from user.
                        
                        YOU MUST GENERATE RESPONSE IN JSON FORMAT AS FOLLOWS:
                        {{"SQL_QUERY": "BOOLEAN YES OR NO | 'YES' based on the intent of the current and past user's query",
            ...
            prompt = ChatPromptTemplate.from_messages(
                [
                    (
                        "system",
                        """You are an expert real estate sales assistant at NHC Housing Company with ARABIC native language who talks with the user as a real person based on the user query, conversation history and current information given below.
                        As an expert real estate agent, avoid using short forms like sqm, SAR, etc. Instead use square meters, Saudi Riyals, etc.
                        Always round off the price for eg. 1209856 to 1.2 million.
                        user query: {user_query}
                        conversation history: {chat_history}
                        ...
                        You need to strictly follow below guidelines/ checklist:
                        1. If chat history is none, start with the greeting message "مرحبا بكم في NHC ، أنا مساعد الذكاء الاصطناعي الشخصي الخاص بك! كيف يمكنني مساعدتك اليوم في العثور على أفضل عقاراتك؟"
                        2. If user's name is provided in the conversation history, address the user by name. BUT DON'T USE IT REPEATEDLY.
                        3. Always continue the conversation and do not repeat the information already provided in the chat history unless user specifically asks for it.
                        4. ONLY USE `current information` to provide information about the properties. NEVER USE INFORMATION NOT PRESENT IN `current information`.
                        5. Never overload the user with excess information, always provide short and only relevant information like price, number of rooms, completion status, etc. based on the user query and current information.
                        6. Never mention any type of ID or code as it is irrelevant to the user.
